...
@section('title', 'Manage Co-Hosts - ' . $hotel->description)
@section('mainbody')
                ...
                <div class="nk-block-head nk-block-head-sm">
                    <div class="nk-block-between">
                        <div class="nk-block-head-content">
                            <h3 class="nk-block-title page-title">Co-Hosts Management</h3>
                            <div class="nk-block-des text-soft">
                                <p>Manage co-hosts for <strong>{{ $hotel->description }}</strong></p>
                            </div>
                        </div>
                        <div class="nk-block-head-content">
                            <a href="{{ route('vendor.co-hosts.create', $hotel->id) }}" class="btn btn-primary">
                ...
                <div class="card">
                    <div class="card-body">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <small class="text-muted">{{ $coHost->response_time }}</small>
                                        </td>
                                        <td>{{ $coHost->email }}</td>
                                        <td>{{ $coHost->phone ?? 'N/A' }}</td>
                                        <td>{{ $coHost->language ?? 'English' }}</td>
                                        <td style="text-align: center;">
                                            <span class="badge badge-success" style="font-size: 14px;">{{ $coHost->response_rate }}%</span>
                                        </td>
                                        <td style="text-align: center;">
                                            @if($coHost->is_active)
                                        ...
                                        <td style="text-align: center;">
                                            <a href="{{ route('vendor.co-hosts.edit', [$hotel->id, $coHost->id]) }}" 
                                            ...
                                            <form id="toggle-status-{{ $coHost->id }}" 
                                                  action="{{ route('vendor.co-hosts.update', [$hotel->id, $coHost->id]) }}" 
                                            ...
                                            <button type="button" 
                                                    ...
                                                    onclick="deleteCoHost({{ $coHost->id }})"
                                                    title="Delete">
                                                <i class="fas fa-trash"></i> Delete
                                            </button>
                                            
                                            <form id="delete-form-{{ $coHost->id }}" 
                                                  action="{{ route('vendor.co-hosts.destroy', [$hotel->id, $coHost->id]) }}" 
                        ...
                        <div class="text-center" style="padding: 60px 20px;">
                            ...
                            <p style="color: #999; margin-bottom: 30px;">Add co-hosts to help manage this property professionally</p>
                            <a href="{{ route('vendor.co-hosts.create', $hotel->id) }}" class="btn btn-primary btn-lg">
                                <em class="icon ni ni-plus"></em>
                                <span>Add Your First Co-Host</span>
                            </a>
                        </div>
                        @endif
                    </div>
...
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
<script>
    function deleteCoHost(coHostId) {
        Swal.fire({
            title: 'Are you sure?',
