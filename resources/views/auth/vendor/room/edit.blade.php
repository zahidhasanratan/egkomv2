    ...
    <div class="nk-content">
                                        ...
                                        <div class="tab-pane fade {{ old('active_tab', 'tabItem3') === 'Photos' ? 'show active' : '' }}" id="Photos">
                                            <div class="row gy-4">
                                                ...
                                                @foreach ($photo_categories as $input_name => $label)
                                                    @php
                                                        $category_key = str_replace('_photos', '', $input_name);
                                                        $has_photos = isset($room_photos[$category_key]) && $room_photos[$category_key]->count() > 0;
                                                    @endphp
                                                    <div class="col-md-6 col-lg-4 col-xxl-3">
                                                        <div class="form-group mt-15">
                                                            <label class="form-label d-block mb-1">{{ $label }}</label>
                                                            <div class="mb-2 photo-delete-all-wrap" style="{{ $has_photos ? '' : 'display: none;' }}">
                                                                <button type="button" class="btn btn-sm btn-outline-danger multiple-remove-all-btn" data-field="{{ $input_name }}" title="Remove all photos in this category">
                                                                    <i class="fa fa-trash-o"></i> Delete All Photos
                                                                </button>
                                                            </div>
                                                            <div class="multiple-upload-container" id="upload-container-{{ $category_key }}" data-field="{{ $input_name }}">
                                                                <input type="file" class="multiple-file-input" id="file-input-{{ $category_key }}" name="{{ $input_name }}[]" accept="image/*" multiple>
                                                                <label class="upload-label" for="file-input-{{ $category_key }}">+ Add More Images</label>
                                                                <div class="multiple-thumbnail-gallery">
                                                                    @if (isset($room_photos[$category_key]))
                                                                        @foreach ($room_photos[$category_key] as $photo)
        ...
        <script>
                        ...
                        .then(data => {
                            ...
                            if (typeof Swal !== 'undefined') {
                                ...
                                if (container && container.querySelectorAll('.multiple-thumbnail-item').length === 0) {
                                    var w = container.closest('.form-group').querySelector('.photo-delete-all-wrap');
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                window._roomEditUploadedImages = uploadedImages;
                ...
                document.addEventListener('click', function(e) {
                    if (e.target.classList.contains('multiple-remove-all-btn') || (e.target.closest && e.target.closest('.multiple-remove-all-btn'))) {
                        ...
                        if (!container) return;
                        var items = container.querySelectorAll('.multiple-thumbnail-item');
                        var toDelete = [];
                        items.forEach(function(el) {
                            var id = el.getAttribute('data-photo-id');
                            if (id) toDelete.push({ id: id, el: el, btn: el.querySelector('.multiple-remove-btn') });
                            else el.remove();
                        });
                        var doDeleteAll = function() {
                            var fileInput = container.querySelector('.multiple-file-input');
                            if (fileInput) fileInput.value = '';
                            var containerId = container.id;
                            if (containerId && window._roomEditUploadedImages) window._roomEditUploadedImages[containerId] = [];
                            var wrap = btn.closest('.photo-delete-all-wrap');
                            if (wrap) wrap.style.display = 'none';
                            toDelete.forEach(function(o) {
                                deletePhotoRequest(o.id, o.btn);
                            });
                        };
                        ...
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({
                                ...
                                text: 'All photos in this category will be removed. This cannot be undone.',
                                icon: 'warning',
                                showCancelButton: true,
                                confirmButtonColor: '#dc3545',
                                cancelButtonColor: '#6c757d',
                                ...
                                if (result.isConfirmed) doDeleteAll();
                            });
                        } else {
