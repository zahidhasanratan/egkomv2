        ...
        try {
            ...
            if ($shouldRecalculate || true) { // Force recalculation every time
                ...
                if ($firstRoom) {
                    ...
                    // Use newTotalNights which was calculated above - this is the correct value
                    $totalNights = intval($newTotalNights);
                    
                    // Debug: Log the calculation values
                    \Log::info('Booking Price Recalculation', [
                        'booking_id' => $booking->id,
                        'price_per_night' => $pricePerNight,
                        'quantity' => $quantity,
                        'total_nights' => $totalNights,
                        'old_subtotal' => $booking->subtotal,
                        'old_grand_total' => $booking->grand_total,
                    ]);
                    ...
                    \Log::info('Booking Price Updated', [
                        'booking_id' => $booking->id,
                        'new_subtotal' => $booking->subtotal,
                        'new_tax' => $booking->tax,
                        'new_grand_total' => $booking->grand_total,
                    ]);
                }
            }
